<template>
  <div v-if="dataLoaded" class="container my-10 px-4">
    <!-- No Data -->
    ...
    >
      <router-link
        class="flex flex-col items-center bg-gray-800 p-8 shadow-md cursor-pointer"
      ...
      >
        ...
        <p
          class="mt-6 py-1 px-3 text-xs text-white bg-indigo-600 shadow-md rounded-lg"
        >
          {{ workout.workoutType }}
        </p>

        <h1 class="mt-8 mb-2 text-center text-xl text-indigo-500">
          {{ workout.workoutName }}
        </h1>
      </router-link>
    </div>
